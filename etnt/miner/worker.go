...
func (w *worker) commitTransactions(env *environment, txs *types.TransactionsByPriceAndNonce, interrupt *int32) error {
	...
	for {
		...
		// add IsEtntPoWFork check sign with chainid protected force check
		if w.chainConfig.IsEtntPoWFork(env.header.Number) && !tx.Protected() {
			log.Trace("Ignoring not protected transaction", "hash", tx.Hash(), "eipEthPoW", w.chainConfig.EtntPoWForkBlock)
			txs.Pop()
			continue
		}
